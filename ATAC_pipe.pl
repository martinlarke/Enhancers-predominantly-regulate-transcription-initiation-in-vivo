##########################################################################################################################################
#  A script for unzipping, aligning, deduplicating and converting ATAC-seq fastq files into coverage tracks (adjusts for Tn5 cutting bias)
#
#                       To run, place in folder with zipped paired end R1.fastq.gz and R2.fastq.gz file and type:
#
#                                               nohup perl ATAC_pipe.pl <$genome> & 
#
...
# convert properpairs.bam into a bedpe file
...
# make frags.bed 
my $awk1= '{if ($6=="+") {print $1"\t"$2"\t"$5"\t""0""\t""0""\t""+" > "frags.plus.bed"} else if ($6=="-") {print $1"\t"$4"\t"$3"\t""0""\t""0""\t""-" > "frags.minus.bed"}}';
system ("nohup awk '$awk1' TEMP_bedpe_fragments.txt");
print "temporary fragments file converted into frags.bed\n\n";

# sort frags file
system ("nohup sort -k1,1 -k2,2n frags.plus.bed > frags.plus.srt.bed");
system ("nohup sort -k1,1 -k2,2n frags.minus.bed > frags.minus.srt.bed");
print "frags files sorted\n\n";

# get genome file
system ("fetchChromSizes $genome > $genome.genome");
print "$genome genome file retrieved\n\n";

# slop frags to account for Tn cutting
system ("nohup bedtools slop -l 4 -r 4 -i frags.plus.srt.bed -g $genome.genome > frags.plus.srt.slop.bed");
system ("nohup bedtools slop -l -5 -r -5 -i frags.minus.srt.bed -g $genome.genome > frags.minus.srt.slop.bed");
print "frag files sloped\n\n";

# cat frag files
system ("nohup cat frags.plus.srt.slop.bed frags.minus.srt.slop.bed > frags.slop.bed");
print "stranded frag files concatenated\n\n";

#remove chr(XYM) lines
my $grep = "chr[a-zA-Z]";
system ("nohup grep -v '$grep' frags.slop.bed > frags.slop.nochrXYM.bed");
print "chrXYM lines removed from frags\n\n";

# sort
system ("nohup sort -k1,1 -k2,2n frags.slop.nochrXYM.bed > frags.slop.nochrXYM.srt.bed");
print "no chrXYM frags sorted\n\n";

#count
system "(nohup wc -l frags.slop.nochrXYM.srt.bed > frags.srt.slop.srt.nochrXYM.srt.count)";
print "no chrXYM frags counted\n\n";

########## After counting total reads, calcuate scaling factor (1/millions of mapped reads) and apply to bedGraph generation
...
# cleanup temp files
#system ("rm -rf *L00* *.fastq *.sam properpairs.bam filteredpairs.bam frags.bed fullfrag.bg");
